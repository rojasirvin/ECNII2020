...
- Es un típico problema de endogeneida

- Estamos estimando una relación de equilibrio

- Nos interesa $\beta_1$. la elasticidad de la oferta laboral
...
- $Z_j$ representa, para cada unidad $j$ (ciudad, localidad), el crecimiento del empleo que hubiera ocurrido si cada industria en $j$ hubiera crecido hubiera crecido al mismo ritmo que su contraparte a nivel nacional

- **Share**: $s_{ij}$ es la partición del trabajo empleado en la industria $i$ en el total del trabajo de la región $j$, con  $\sum_{i=1}^I s_{ij}=1$

- **Shift**: $G_j$ es la tasa de crecimiento del empleo en la industria $j$ a nivel nacional
...
$$\Delta y_{it}=y_{it-1}g_{it}=y{it-1}(G_t+(G{it}-G_t)+(g_{it}-G_{it}))$$
...
- Esta expresión parte el crecimiento de la variable de interés en tres componentes (la terminología en nigritas es de McMillen, 2010)
...
&=\sum_{j=j}^J e_{ijt-1}(G_t+(G_{it}-G_t)) \\
