...
```{r tidy=T, echo=T, include=T, message=F}
mlogit <- glm(charter ~ lnrelp, family = binomial(link = "logit"), 
    data = data_binary)

mlineal <- lm(charter ~ lnrelp, data=data_binary)
...
data_binary<-cbind(data_binary,plogit,pprobit,plineal)
data_for_plot <- melt(data_binary ,  id.vars = 'lnrelp', variable.name = 'Modelo',measure.vars = c("plogit","pprobit","plineal"), value.name = "prob")
